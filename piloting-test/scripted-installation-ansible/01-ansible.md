...
12. **Встановити компоненти** в наступній послідовності:
  ...
  - **Zabbix Agent2 для MonHub рішення**
     ...
     ```
> ⚠️ Перш ніж встановлювати UXP Security Server (ШБО), будь ласка, перевірте мережеві налаштування на сервері. Для коректного встановлення необхідно, щоб було налаштоване повне доменне ім’я (FQDN).
> 
> ```bash
> hostname -A
> ```
...
> ```
  ...
  - **MinIO**
     ...
     ```
> ⚠️ **Після встановлення MinIO виконайте пункт за посиланням:** [Перевірка параметра archive-storage-type](../manual-installation/04-minio-install-and-settings.md#перевірка-параметра-archive-storage-type)
