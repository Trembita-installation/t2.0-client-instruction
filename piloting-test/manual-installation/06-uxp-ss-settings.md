...
> ```MemberCode_SS_Env2_Number_FreeSymbols```, де: <br>
> ```MemberCode``` — код ЄДРПОУ Учасника системи "Трембіта"; <br>
> ```Env2``` — код середовища (```T2``` — тестове); <br>
...
Даний крок "CSR сервера" пропускаємо, так як потрібно підключити "Криптографічний пристрій"
по інструкції [Підключення криптографічного пристрою](05-uxp-ss-install.md#aparatni-tokeny)

Далі в розділі "Ключі та сертифікати" згенеруйте два ключі та відповідні CSR-запити:
...
Згенеруйте CSR-запит для сертифіката автентифікації. CSR-запит завантажаться автоматично.<br>
**В пункті "3. Оберіть центр сертифікації" вибираємо "Central certification authority"** <br>
...
Якщо буде використовуватись файловий ключ - згенеруйте CSR-запит для сертифікату сертифіката підписання. CSR-запит завантажаться автоматично.<br>
**В пункті "3. Оберіть центр сертифікації" вибираємо "Central certification authority"** <br>
...
![](06-uxp-ss-settings/image28.png)Статус клієнта зміниться на "Відбувається реєстрація".
...
| Статус | Опис |
| --- | --- |
| ![](06-uxp-ss-settings/image33.png) | **Збережено** – інформацію про клієнта було додано на ШБО |
| ![](06-uxp-ss-settings/image34.png) | **Відбувається реєстрація** – було надіслано запит на реєстрацію клієнта |
| ![](06-uxp-ss-settings/image35.png) | **Зареєстровано** – клієнт може починати обмінюватись інформацією |
