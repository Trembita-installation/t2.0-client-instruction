...
Переконайтесь, що ви налаштували свій мережевий екран на дозвіл лише вхідних з’єднань через TCP порти, як вказано у розділі ____. Запустіть сканування портів з різних зон доступу (із зовнішньої мережі, підмережі внутрішньої мережі, а також з робочого місця адміністратора, щоб цілком переконатися, що всі налаштування виконано коректно):

```bash
nmap --open <security-server>
```

🔹 Зовнішня мережа: відкритий лише порт TCP 5500
