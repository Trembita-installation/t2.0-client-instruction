...
🔸 За потреби, у візуалізаціях застосовуйте фільтр, щоб уникнути дублювання записів з боку клієнта і сервісу
...
Приклад ```uxp-monitor-analytics``` постачається з прикладом інформаційної панелі:
...
✅ Готово. Панель з’явиться у розділі ```Analytics``` → ```Dashboard```.
