# Встановлення пакета uxp-securityserver-ua та залежностей (ШБО)
Шлюз безпечного обміну (ШБО) - забезпечує швидку, безпечну й контрольовану передачу даних між системами, з шифруванням та гнучким керуванням доступів до сервісів.
---
...
Шлях до TLS-сертифікату, який був згенерований на MinIO 
<br>
...
>
> • Перевірка цілісності вручну (зупиняє сервіси при помилці):
> ```bash
...
4. Пересилання журналів (опційно)
ШБО підтримує можливість пересилання журналів через rsyslog для передавання логів у зовнішні системи моніторингу та зберігання.

4.1. Якщо у вас вже є встановлений Graylog:

Використовуйте розділ "Пересилання журналів за допомогою Rsyslog" в ______

4.2. Якщо у вас ще немає Graylog, скористайтесь інструкцією в _______

> ℹ️ **Примітка:** Підтримується пересилання журналів до інших систем, але Graylog є рекомендованим рішенням.
...
Встановіть на ШБО програмне забезпечення від виробника пристрою:
[Налаштування ІІТ Гряда-301](#iit-gryada301)
